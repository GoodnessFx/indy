...
export default function Vehicles() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F7F5]">
      {/* Model selector rail */}
      <div className="fixed lg:sticky top-16 lg:top-20 z-30 bg-[#F7F7F5]/95 backdrop-blur-xl border-b border-black/5 w-full">
      ...
      {/* Full-bleed cinematic hero per model */}
      <div className="relative min-h-[82vh] flex items-end overflow-hidden" key={active.id}>
        <img
          src={`https://images.unsplash.com/${active.image}?w=1920&h=1080&fit=crop&auto=format`}
          alt={active.name}
          className="absolute inset-0 w-full h-full object-cover fade-in-up"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#F7F7F5] via-white/40 to-white/30" />

        <div className="relative max-w-[1440px] mx-auto px-6 lg:px-12 w-full pb-12 lg:pb-16 pt-40">
          <p className="font-mono text-xs text-[#2F6BFF] tracking-widest uppercase mb-4 fade-in-up">Vehicle-Backed Fleet</p>
          <h1 className="font-display font-800 text-6xl lg:text-8xl text-[#0A0B0D] tracking-tight mb-4 fade-in-up">
            {active.name}
          </h1>
          <p className="text-black/60 text-base lg:text-lg max-w-xl mb-8 fade-in-up">{active.tagline}</p>

          {/* Spec strip */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-black/10 rounded-2xl overflow-hidden border border-black/10 max-w-3xl mb-4">
            {specs.map(s => (
              <div key={s.label} className="bg-[#F7F7F5]/80 backdrop-blur-md px-5 py-4">
                <div className="flex items-center gap-1.5 text-black/40 mb-1.5">
                  <s.icon size={14} />
                  <span className="text-[11px] uppercase tracking-wide">{s.label}</span>
                </div>
                <p className="font-mono font-600 text-lg text-[#0A0B0D]">{s.value}</p>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <p className="font-mono text-black/60 text-sm">
              From <span className="text-[#0A0B0D] font-600 text-base">${active.price.toLocaleString()}</span>
            </p>
      ...
      <div className="border-t border-black/5 bg-[#F7F7F5]">
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-16 lg:py-24">
          <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-24 items-start">
            <div>
              <p className="font-mono text-xs text-[#2F6BFF] tracking-widest uppercase mb-3">Fractional Exposure</p>
              <h2 className="font-display font-700 text-3xl lg:text-5xl text-[#0A0B0D] mb-6">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={card.title} className="rounded-2xl border border-black/8 bg-white p-6">
                  <div className="w-10 h-10 rounded-xl bg-[#2F6BFF]/12 flex items-center justify-center mb-4">
                    <card.icon size={18} className="text-[#2F6BFF]" />
                  </div>
                  <h3 className="font-display font-600 text-sm text-[#0A0B0D] mb-2">{card.title}</h3>
                  <p className="text-xs text-black/40 leading-relaxed">{card.body}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-12 rounded-2xl border border-black/8 bg-white px-6 lg:px-8 py-6 flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
            <div className="shrink-0">
              <p className="text-xs text-black/30 mb-1">Allocation note, {active.name}</p>
              <p className="text-sm text-black/70 max-w-xl">{active.note}</p>
            </div>
            <div className="md:ml-auto flex items-center gap-2 shrink-0">
              <span className="w-1.5 h-1.5 rounded-full bg-[#22C55E] dot-pulse" />
              <span className="text-xs text-black/40">Allocation window open</span>
            </div>
          </div>
        </div>
      </div>
