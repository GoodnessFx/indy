...
const threads = [
  ...
  { id: 'TK-2839', user: 'Lena Müller', subject: 'Unable to add bank account', status: 'Open', last: '5h ago', unread: true },
  ...
  return (
    <AdminLayout>
      <div className="max-w-6xl -m-8 h-screen flex overflow-hidden">
        {/* Thread list */}
        <div className="w-72 shrink-0 border-r border-white/5 flex flex-col bg-[#070810]">
          <div className="p-4 border-b border-white/5">
            <h2 className="font-mono text-xs text-[#C5C8D0]/50 uppercase tracking-wider mb-3">Support Inbox</h2>
              ...
              {['All', 'Open', 'Waiting on you', 'Resolved'].map(s => (
                <button key={s} onClick={() => setStatusFilter(s)}
                  className={`text-[9px] px-2 py-1 rounded font-mono transition-colors whitespace-nowrap ${
                    statusFilter === s ? 'bg-[#2F6BFF]/20 text-[#2F6BFF]' : 'text-[#C5C8D0]/25 hover:text-[#C5C8D0]/50'
                  }`}>
                  {s}
                </button>
              ))}
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredThreads.map(t => (
              <button key={t.id} onClick={() => setSelected(t.id)}
                className={`w-full text-left px-4 py-3.5 border-b border-white/3 hover:bg-white/2 transition-colors ${selected === t.id ? 'bg-white/3' : ''}`}>
                <div className="flex items-start justify-between gap-2 mb-1">
                  <div className="flex items-center gap-1.5">
                    {t.unread && <div className="w-1.5 h-1.5 rounded-full bg-[#2F6BFF] shrink-0" />}
                    <span className="font-mono text-xs text-[#C5C8D0]/70 truncate max-w-[120px]">{t.user}</span>
                  </div>
                  <span className="font-mono text-[9px] text-[#C5C8D0]/20 shrink-0">{t.last}</span>
                </div>
                <p className="font-mono text-[10px] text-[#C5C8D0]/40 truncate">{t.subject}</p>
        ...
        {currentThread && (
          <div className="flex-1 flex flex-col">
            <div className="px-6 py-4 border-b border-white/5 bg-[#070810]">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-mono text-sm text-[#C5C8D0]/80">{currentThread.subject}</p>
                  <p className="font-mono text-xs text-[#C5C8D0]/30">{currentThread.id} · {currentThread.user}</p>
                </div>
              ...
              {currentMessages.map((msg, i) => (
                <div key={i} className={`flex ${msg.from === 'support' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] rounded-xl px-4 py-3 text-xs font-mono ${
                    msg.from === 'support' ? 'bg-[#2F6BFF]/20 text-[#2F6BFF]' : 'bg-white/5 text-[#C5C8D0]/70'
                  }`}>
                    <p>{msg.text}</p>
            ...
            <div className="p-4 border-t border-white/5 bg-[#070810]">
              <div className="flex items-center gap-3">
                <input value={reply} onChange={e => setReply(e.target.value)}
                  onKeyDown={e => e.key === 'Enter' && sendReply()}
                  placeholder="Type a reply..."
                  className="flex-1 bg-white/3 border border-white/8 rounded-lg px-4 py-2.5 text-xs text-[#C5C8D0] font-mono outline-none focus:border-[#2F6BFF]/40" />
