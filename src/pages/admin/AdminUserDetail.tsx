  ...
  return (
    <AdminLayout>
      <div className="max-w-4xl">
        <Link to="/admin/users" className="flex items-center gap-2 text-xs text-black/30 hover:text-black/70 mb-6 font-mono">
          <ArrowLeft size={12} /> Back to users
        </Link>
        <h1 className="font-mono font-700 text-xl text-[#0A0B0D] mb-6">{user.name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white border border-black/5 rounded-xl p-5">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={field.label}>
                  <label className="block text-[10px] text-black/30 mb-1 font-mono">{field.label.toUpperCase()}</label>
                  <input defaultValue={field.value} readOnly={field.readonly}
                    className={`w-full bg-black/3 border border-black/5 rounded-lg px-3 py-2 text-xs text-[#0A0B0D] font-mono outline-none ${
          ...
          {/* Portfolio & balance */}
          <div className="bg-white border border-black/5 rounded-xl p-5">
            <h3 className="font-mono text-xs text-black/50 uppercase tracking-wider mb-4">Portfolio balances (read-only)</h3>
            <div className="space-y-2 mb-5">
              {[
                ...
                <div key={b.label} className="flex justify-between py-2 border-b border-black/3">
                  <span className="text-xs text-black/40 font-mono">{b.label}</span>
                  <span className="text-xs text-black/70 font-mono">{b.val}</span>
                </div>
              ))}
            </div>
              ...
              {!adjusting ? (
                ...
                <div className="space-y-2">
                  ...
                  <textarea value={adjustReason} onChange={e => setAdjustReason(e.target.value)}
                    ...
                    className="w-full bg-black/3 border border-black/8 rounded-lg px-3 py-2 text-xs text-[#0A0B0D] font-mono outline-none focus:border-[#2F6BFF]/40 resize-none" rows={2} />
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setAdjusting(false)} className="text-xs text-black/30 hover:text-black/60">
                      Cancel
                    </button>
                  </div>
        ...
        <div className="bg-white border border-black/5 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-black/5 flex items-center justify-between">
            <h3 className="font-mono text-xs text-black/50 uppercase tracking-wider">Login history</h3>
          ...
          <div className="divide-y divide-black/3">
            {loginHistory.map((l, i) => (
              <div key={i} className="grid grid-cols-[2fr_1fr_1fr_1fr] px-5 py-3 items-center">
                <span className="text-xs text-black/60 font-mono">{l.device}</span>
                <span className="text-xs text-black/30 font-mono">{l.ip}</span>
                <span className="text-xs text-black/30 font-mono">{l.location}</span>
                <span className="text-xs text-black/25 font-mono">{l.date}</span>
              </div>
            ))}
          </div>
        </div>
