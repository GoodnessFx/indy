...
const activity = [
  ...
  { action: 'New user registered', user: 'Lena Müller', time: '2h ago', type: 'info' },
];
    ...
    <AdminLayout>
      <div className="max-w-6xl">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-mono font-700 text-xl text-[#C5C8D0]">Dashboard</h1>
            <p className="text-xs text-[#C5C8D0]/30 mt-1 font-mono">Sep 23, 2026 · 09:42 UTC</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-[#22C55E] dot-pulse" />
            <span className="text-xs text-[#C5C8D0]/30 font-mono">All systems operational</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
          {kpis.map(kpi => (
            <div key={kpi.label} className="bg-[#0d0f1a] border border-white/5 rounded-xl p-4">
              <div className={`w-8 h-8 rounded-lg ${kpi.bg} flex items-center justify-center mb-3`}>
                <kpi.icon size={14} className={kpi.color} />
              </div>
              <p className="font-mono font-700 text-lg text-[#C5C8D0]">{kpi.value}</p>
              <p className="text-[10px] text-[#C5C8D0]/30 mt-0.5 font-mono">{kpi.label}</p>
              <p className="text-[10px] text-[#C5C8D0]/20 mt-1">{kpi.change}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-[#0d0f1a] border border-white/5 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-white/5">
              <h2 className="font-mono text-sm text-[#C5C8D0]/70">Recent activity</h2>
            </div>
              ...
              {activity.map((a, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-[#C5C8D0]/70 truncate">{a.action} — <span className="text-[#C5C8D0]/40">{a.user}</span></p>
                  </div>
                  <span className="text-[10px] text-[#C5C8D0]/25 font-mono shrink-0">{a.time}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0d0f1a] border border-white/5 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
              <h2 className="font-mono text-sm text-[#C5C8D0]/70">Recent users</h2>
            ...
            <div className="divide-y divide-white/5">
              {adminUsers.map(user => (
                <div key={user.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-[#C5C8D0]/70 truncate">{user.name}</p>
                    <p className="text-[10px] text-[#C5C8D0]/25 font-mono">{user.email}</p>
                  </div>
